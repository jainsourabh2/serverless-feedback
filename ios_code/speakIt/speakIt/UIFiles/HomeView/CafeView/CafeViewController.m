...
- (void)getCafeData
{
    ...
    [manager GET:[NSString stringWithFormat:@"%@places/locationlisting",WEBSERVICEURL] parameters:params progress:nil success:^(NSURLSessionTask *task, id responseObject) {
        ...
        arrOfCafe = [tempDict objectForKey:@"results"];
        
        //Write out the data to the console.
        NSLog(@"Google Data: %@", arrOfCafe);
        [_tableForCafe reloadData];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    NSLog(@"count-%lu",(unsigned long)[arrOfCafe count]);
    return [arrOfCafe count];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CafeTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"CafeTableViewCell" forIndexPath:indexPath];
    
    // Configure the cell...
    
    cell.lblName.text = [[arrOfCafe objectAtIndex:indexPath.row] objectForKey:@"name"];
    cell.lblDetail.text = [[arrOfCafe objectAtIndex:indexPath.row] objectForKey:@"vicinity"];
    
    cell.imageView.contentMode = UIViewContentModeScaleAspectFit;
    [cell.imgForPhoto sd_setImageWithURL:[NSURL URLWithString:[arrOfCafe[indexPath.row] objectForKey:@"icon"]] placeholderImage:[UIImage imageNamed:@"defaultimg.jpg"]];
...
{
    if (tableView == _tableForCafe)
    {
        [tableView deselectRowAtIndexPath:indexPath animated:YES];
        
        CafeTableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
        [self.delegatesCafe goForFeedbackPageCafe:[[arrOfCafe objectAtIndex:indexPath.row] objectForKey:@"name"] isAddress:[[arrOfCafe objectAtIndex:indexPath.row] objectForKey:@"vicinity"] urlIcon:[NSURL URLWithString:[arrOfCafe[indexPath.row] objectForKey:@"icon"]] isPlaceId:[[arrOfCafe objectAtIndex:indexPath.row] objectForKey:@"place_id"] isPhotoRef:[[[[arrOfCafe objectAtIndex:indexPath.row] objectForKey:@"photos"] valueForKey:@"photo_reference"] objectAtIndex:0] isRating:[[arrOfCafe objectAtIndex:indexPath.row] objectForKey:@"rating"]];
