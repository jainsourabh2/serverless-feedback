...
- (void)getBankData
{
    ...
    [manager GET:[NSString stringWithFormat:@"%@places/locationlisting",WEBSERVICEURL] parameters:params progress:nil success:^(NSURLSessionTask *task, id responseObject) {
        ...
        arrOfBank = [tempDict objectForKey:@"results"];
        
        //Write out the data to the console.
        NSLog(@"Google Data: %@", arrOfBank);
        [_tableForBank reloadData];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    NSLog(@"count-%lu",(unsigned long)[arrOfBank count]);
    return [arrOfBank count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.lblName.text = [[arrOfBank objectAtIndex:indexPath.row] objectForKey:@"name"];
    cell.lblDetail.text = [[arrOfBank objectAtIndex:indexPath.row] objectForKey:@"vicinity"];
    
    cell.imageView.contentMode = UIViewContentModeScaleAspectFit;
    [cell.imgForPhoto sd_setImageWithURL:[NSURL URLWithString:[arrOfBank[indexPath.row] objectForKey:@"icon"]] placeholderImage:[UIImage imageNamed:@"defaultimg.jpg"]];
    
    NSLog(@"photo ref:%@",[[[[arrOfBank objectAtIndex:indexPath.row] objectForKey:@"photos"] valueForKey:@"photo_reference"] objectAtIndex:0]);
...
{
    if (tableView == _tableForBank)
    {
        [tableView deselectRowAtIndexPath:indexPath animated:YES];
        
        BankTableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
        //Call delegate method
        [self.delegatesBank goForFeedbackPageBank:[[arrOfBank objectAtIndex:indexPath.row] objectForKey:@"name"] isAddress:[[arrOfBank objectAtIndex:indexPath.row] objectForKey:@"vicinity"] urlIcon:[NSURL URLWithString:[arrOfBank[indexPath.row] objectForKey:@"icon"]] isPlaceId:[[arrOfBank objectAtIndex:indexPath.row] objectForKey:@"place_id"] isPhotoRef:[[[[arrOfBank objectAtIndex:indexPath.row] objectForKey:@"photos"] valueForKey:@"photo_reference"] objectAtIndex:0] isRating:[[arrOfBank objectAtIndex:indexPath.row] objectForKey:@"rating"]];
